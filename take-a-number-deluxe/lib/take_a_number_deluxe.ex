defmodule TakeANumberDeluxe do
  use GenServer
  # Client API

  @spec start_link(keyword()) :: {:ok, pid()} | {:error, atom()}
  def start_link(init_arg) do
    GenServer.start_link(__MODULE__, init_arg)
  end

  @impl GenServer
  def init(init_arg) do
    min_number = Keyword.get(init_arg, :min_number)
    max_number = Keyword.get(init_arg, :max_number)
    auto_shutdown = Keyword.get(init_arg, :auto_shutdown_timeout, :infinity)

    if validate_min_max(min_number, max_number) do
      {:ok, state} = TakeANumberDeluxe.State.new(min_number, max_number, auto_shutdown)

      {:ok, state, state.auto_shutdown_timeout}
    else
      {:stop, :invalid_configuration}
    end
  end

  defp validate_min_max(min_number, max_number) do
    is_number(min_number) && is_number(max_number) && min_number < max_number
  end

  @spec report_state(pid()) :: TakeANumberDeluxe.State.t()
  def report_state(machine), do: GenServer.call(machine, :report_state)

  @spec queue_new_number(pid()) :: {:ok, integer()} | {:error, atom()}
  def queue_new_number(machine), do: GenServer.call(machine, :queue_new_number)

  @spec serve_next_queued_number(pid(), integer() | nil) :: {:ok, integer()} | {:error, atom()}
  def serve_next_queued_number(machine, priority_number \\ nil), do: GenServer.call(machine, {:serve_next_queued_number, priority_number})

  @spec reset_state(pid()) :: :ok
  def reset_state(machine) do
    GenServer.cast(machine, :reset_state)

    :ok
  end

  # Server callbacks

  @impl GenServer
  def handle_call(:report_state, _from, state), do: {:reply, state, state, state.auto_shutdown_timeout}
  @impl GenServer
  def handle_call(:queue_new_number, _from, state) do
    with {:ok, number, new_state} <- TakeANumberDeluxe.State.queue_new_number(state) do
      {:reply, {:ok, number}, new_state, new_state.auto_shutdown_timeout}
    else
      {:error, _error} = err -> {:reply, err, state, state.auto_shutdown_timeout}
    end
  end
  @impl GenServer
  def handle_call({:serve_next_queued_number, priority_number}, _from, state) do
    with {:ok, number, new_state} <- TakeANumberDeluxe.State.serve_next_queued_number(state, priority_number) do
      {:reply, {:ok, number}, new_state, new_state.auto_shutdown_timeout}
    else
      {:error, _error} = err -> {:reply, err, state, state.auto_shutdown_timeout}
    end
  end

  @impl GenServer
  def handle_cast(:reset_state, state) do
    {:ok, state} = TakeANumberDeluxe.State.new(state.min_number, state.max_number, state.auto_shutdown_timeout)
    {:noreply, state, state.auto_shutdown_timeout}
  end

  @impl GenServer
  def handle_info(:timeout, _state) do
    exit(:normal)
  end
  @impl GenServer
  def handle_info(_, state) do
    {:noreply, state, state.auto_shutdown_timeout}
  end
end
